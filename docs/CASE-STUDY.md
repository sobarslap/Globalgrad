...
|---|---|---|---|
| 1 | Coherent per-program data (tuition + CGPA + IELTS + GRE flowing into matching, cost, scholarships) | **Curated real dataset** — 55 real programs across 12 countries with real tuition/CGPA/IELTS/GRE + `minGre` schema field; one dataset feeds matching, cost, and scholarships | Seed data was synthetic (faked tuition/amounts) |
...
- **Unique decision tools** — Reality Check (hidden costs, housing, language), peer-outcome "real odds" (Similar Student Finder), interactive map, compare, insights feed.
...
- **Tested decision engines** — 64 unit tests across matching, readiness, cost, scholarship, country, similar.

---

## Engineering highlights (résumé-ready)

- Built a **rule-based decision-engine layer** (matching, readiness, cost, scholarship, country, similar) as pure, framework-agnostic TypeScript with **64 unit tests** — deterministic, explainable admissions guidance (Safe/Target/Reach with per-program reasoning).
