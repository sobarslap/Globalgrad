...
function TrackButton({ programId }: { programId: string }) {
  ...
  return (
    <button
      ...
      className="mt-3 inline-flex items-center gap-1.5 rounded-md border border-border/60 px-2.5 py-1 text-xs font-medium transition-colors hover:border-primary/50 disabled:opacity-60"
    >
      {added ? (
        <>
...
function ProgramCard({ match }: { match: ProgramMatch }) {
  ...
  return (
    <div className={`rounded-xl border ${meta.ring} bg-card/50 p-4`}>
      ...
      <TrackButton programId={match.program.id} />
    </div>
  );
}
